...
void Texture::setAnimationOnEnd(std::function<void()> listener) {
  animationOnEndHandler_ = std::move(listener);
}
...
void Texture::render(const Vector2D<int>& position) const {
  ...
  const SDL_Rect rectangle{position.getX(), position.getY(), size.getX(),
                           size.getY()};
  render(rectangle);
}
